...
tldr: 针对代理强化学习（Agentic RL）中外部资源（如代码执行和奖励模型）利用率低的问题，本文提出 ARL-Tangram 系统。该系统引入动作级编排机制，通过统一的动作表述和弹性调度算法实现细粒度的资源共享与动态扩展。实验证明，ARL-Tangram 显著缩短了动作完成时间并大幅节省了资源，已成功应用于 MiMo 系列模型的训练。
selection_source: fresh_fetch
motivation: 现有的代理强化学习框架因采用静态资源预留模式，导致外部异构资源在处理复杂交互任务时存在严重的效率瓶颈。
method: 提出动作级编排架构，利用弹性调度算法和异构资源管理器实现细粒度的资源共享与任务调度。
result: "该系统将平均动作完成时间提升了 4.3 倍，训练速度加快 1.5 倍，并节省了超过 70% 的外部资源。"
conclusion: ARL-Tangram 有效解决了代理强化学习中的资源低效问题，是支持大规模智能体模型训练的高效基础设施。
---
