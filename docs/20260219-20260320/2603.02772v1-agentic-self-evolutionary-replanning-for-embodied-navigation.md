...
tldr: 针对具身导航中重规划模型固定、缺乏自适应能力的问题，本文提出SERP框架。该框架通过运行时学习实现模型自进化，引入ILAD机制进行参数动态调整与重置，并结合GCOT技术在蒸馏图上进行高效推理。实验证明，SERP在提升导航成功率的同时显著降低了Token消耗，增强了机器人在复杂环境中的鲁棒性。
selection_source: fresh_fetch
motivation: 现有的具身导航重规划方法通常使用固定的动作模型，无法通过实时经验学习来优化自身以应对复杂的失败场景。
method: 提出SERP框架，利用带自动微分的上下文学习（ILAD）实现模型自进化，并结合图思维链（GCOT）在蒸馏图上进行高效重规划。
result: 在仿真和真实世界实验中，SERP在多种基准测试下均实现了更高的导航成功率和更低的Token开销。
conclusion: SERP通过引入自进化机制和高效的图推理，为提升具身智能体的鲁棒性和资源效率提供了有效的解决方案。
---
